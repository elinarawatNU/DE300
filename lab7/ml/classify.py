...
# NOTE: added this
class Lab7Task(Transformer):
    def __init__(self, input_cols, output_cols):
        self.__input_cols = input_cols
        self.__output_cols = output_cols

    def transform(self, df):
        for input_col, output_col in zip(self.__input_cols, self.__output_cols):
...
def pipeline(data: DataFrame):
    ...
    # NOTE: ADDED THIS
    square_col = [f"{sq_col}" for sq_col in imputed_columns_numeric]
    sq_features = Lab7Task(input_cols=imputed_columns_numeric, output_cols=square_col)


    # Assemble feature columns into a single feature vector
    # NOTE: changed this
    assembler = VectorAssembler(
        inputCols=pairwise_columns + square_col + imputed_columns_numeric + imputed_columns_string, 
        outputCol="features"
        )
    ...
    # Create the pipeline
    pipeline = Pipeline(stages=[indexer, *imputers_string, imputer_numeric, pairwise_product, sq_features, assembler, classifier]) # NOTE: changed this
